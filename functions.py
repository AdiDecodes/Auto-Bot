...
def calctime():
    ...
    time.sleep(0.2)
    with keyboard.pressed(copy_button):
        keyboard.press('c')
    pointer_click(BLANK)
    newTime = pyperclip.paste()
    newTime = ''.join(i for i in newTime if i.isdigit() or i == '.')
    return int(float(str(newTime)))
...
def setTime():
    ...
    # timeframe = calctime()
    # if ((timeframe - int(time_now.strftime('%M')) == 1)):
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    # elif (timeframe - int(time_now.strftime('%M')) == 2):
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    # elif (timeframe - int(time_now.strftime('%M')) == 3):
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    # elif (timeframe - int(time_now.strftime('%M')) == 4):
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    # elif (timeframe - int(time_now.strftime('%M')) == 5):
    #     pointer_click(PLUS)
    # elif (timeframe - int(time_now.strftime('%M')) == 0):
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    #     pointer_click(PLUS)
    pointer_click(TIME_BOX)
    pointer_click(TIME_SELECT)
    print("Time Interval Calibrated")
...
#     @client.on(events.NewMessage(chats='testaccess1'))
#     async def my_event_handler(event):
#         print(event.raw_text)
#         if event.raw_text.find("DOWN") == -1:
#             Signal = str("UP")
#             print("Up")
#         else:
#             Signal = str("DOWN")
...
def trade(var_static, var_return):
    ...
    if m_max > 0:
        m_level = martingale(signal, op_list, m_level, m_max, m_switching)
    ...
    print(f'\n-------------------------')
    print(f"\n{'CURRENT BALANCE:':20}", 'Rp. '
          f'{balance:,}')
    print(f"{'PRIOR BALANCE:':20}", 'Rp. '
          f'{balance_before:,}')
    print(f"{'PROFIT/LOSS:':20}", 'Rp. '
...
def get_csv_data(strategy, m_max, balance):
    ...
    col_date, col_time_start, col_time_end = list(df['Date']), list(
        df['Time Start']), list(df['Time End'])
    col_strategy, col_martingale_max = list(df['Strategy']), list(
        df['Martingale Max'])
    col_balance_start, col_balance_end = list(df['Starting Balance']), list(
        df['Ending Balance'])
    ...
    return [
        col_date, col_time_start, col_time_end, col_strategy,
        col_martingale_max, col_balance_start, col_balance_end, col_trades,
        col_martingales
    ]
...
def set_csv_data(data, balance, trades_list, martingales_list):
    ...
    if len(trades_list) == 1:
        ...
        col_time_end[len(col_time_end) -
                     1] = datetime.now().strftime('%H:%M:%S')
    ...
    df = pd.DataFrame()
    df['Date'], df['Time Start'], df[
        'Time End'] = col_date, col_time_start, col_time_end
    df['Strategy'], df['Martingale Max'] = col_strategy, col_martingale_max
    df['Starting Balance'], df[
        'Ending Balance'] = col_balance_start, col_balance_end
